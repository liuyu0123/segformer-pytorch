...
from segformer_water import SegFormer_Segmentation
...
if __name__ == "__main__":
    ...
    blend           = True  # 设置为False则只输出分割mask，不混合原图
    #-------------------------------------------------------------------------#
    ...
    if mode == "predict":
        '''
        单张图片预测模式
        1、该代码无法直接进行批量预测，如果想要批量预测，可以利用os.listdir()遍历文件夹
        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                ...
                # blend=False 可以只保存分割结果，不混合原图
                r_image = segformer.detect_image(image, blend=blend)
        ...
        raise AssertionError("Please specify the correct mode: 'predict', 'video', 'fps', 'dir_predict' or 'export_onnx'.")